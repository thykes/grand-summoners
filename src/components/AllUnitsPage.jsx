...
export default function AllUnitsPage({
  ...
  const filteredAndSortedUnits = useMemo(() => {
    let processedUnits = showMyUnitsFilter
      ? allUnits.filter((u) => u && myUnits.has(u.id))
      : [...allUnits];

    processedUnits = processedUnits.filter((unit) => {
      if (!unit || !unit.Unit_Name) return false;
  ...
  const handleClearAll = () => {
    if (window.confirm('Are you sure you want to remove all units from your collection?')) {
      onClearAll?.();
    }
  };
        ...
        <div className="card mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            ...
            <div className="flex items-center justify-center p-2 bg-gray-900/50 rounded-md border border-gray-700">
              <label
                htmlFor="my-units-toggle"
                className="mr-3 text-sm font-bold text-gray-300 select-none"
              >
                Show My Units
              </label>
              <input
                type="checkbox"
                id="my-units-toggle"
                checked={showMyUnitsFilter}
                onChange={(e) => setShowMyUnitsFilter(e.target.checked)}
